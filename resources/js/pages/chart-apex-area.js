...
import { githubdata } from "../data/github-data"
import { dataSeries } from "../data/irregular-data-series"
...
// Splite Area
//
const splineAreaChart = document.querySelector("#spline-area")
if (splineAreaChart) {
    new CustomApexChart({
        selector: splineAreaChart,
        options: () => ({
            chart: {
                height: 380,
                type: "area",
            ...
            colors: [theme("chart-alpha"), theme("chart-beta")],
            series: [
                {
                    name: "iPhone 16",
...
//
const datetimeChart = new CustomApexChart({
    selector: "#area-chart-datetime",
    options: () => ({
        annotations: {
            yaxis: [
                {
...
const fullData = datetimeChart.getOptions().series[0].data

// Filter and update chart
function filterChartRange(range) {
    const now = new Date(fullData[fullData.length - 1][0])
    let fromDate
    ...
    switch (range) {
        ...
        case "ALL":
        default:
            fromDate = new Date(fullData[0][0])
            break
    }

    const filtered = fullData.filter((point) => point[0] >= fromDate.getTime())

    datetimeChart.chart.updateSeries([
        {
            name: "Paces",
            data: filtered,
        },
    ...
    options: () => ({
        chart: {
            ...
            selection: {
                enabled: true,
                xaxis: {
                    min: new Date("05 Jan 2015").getTime(),
                    max: new Date("04 Jan 2016").getTime(),
                },
                fill: {
...
function generateWaveStyleData(baseTime, count, min, max) {
    let series = []
    for (let i = 0; i < count; i++) {
        let value = (Math.sin(i / 2) * (max - min)) / 2 + (min + (max - min) / 2)
        value = Math.max(0, parseFloat(value.toFixed(2))) // ensure positive only
...
new CustomApexChart({
    ...
    options: () => ({
        ...
        colors: [theme("chart-beta"), theme("chart-primary"), theme("chart-secondary")],
        markers: {
            size: 0,
            style: "full",
        },
...
//
new CustomApexChart({
    selector: "#area-chart-nullvalues",
    options: () => ({
        chart: {
            height: 380,
            type: "area",
