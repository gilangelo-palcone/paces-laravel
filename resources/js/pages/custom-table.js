...
class Table {
            ...
            checkboxes.forEach((checkbox) => {
                checkbox.addEventListener("change", () => {
                    ...
                    if (this.deleteSelectedSelector && this.filteredRows.length > 0) {
                        this.deleteSelectedSelector.classList.toggle("hidden", !anyChecked)
                    }
                })
            })
        }
    ...
    renderTablePage(page) {
        ...
        if (pageRows.length === 0) {
            ...
            messageRow.innerHTML = `<td colspan="${columnCount}" class="text-center text-default-400 py-3">${this.itemNotFoundMessage}</td>`
    ...
    setupPaginationInfo() {
        ...
        if (this.paginationInfo) {
            this.paginationInfo.className = "text-default-400"
            ...
            this.paginationInfo.innerHTML = `Showing <span class="font-semibold">${startRow}</span> to <span class="font-semibold">${endRow}</span> of <span class="font-semibold">${this.filteredRows.length}</span> ${infoAttribute && infoAttribute !== "" ? infoAttribute : "entries"}`
        }
    }
    ...
    setupSort() {
        ...
        sortItems.forEach((header) => {
            ...
            if (!icon) {
                ...
                icon.classList.add("inline-flex", "align-middle", "h-3.75")
          ...
          <path d="M21 15l-4 4l-4 -4m4 4v-14" />
            ...
            header.addEventListener("click", (e) => {
                ...
                if (newDirection === "asc") {
                    icon.innerHTML = `<svg  xmlns="http://www.w3.org/2000/svg"  width="13"  height="13"  viewBox="0 0 24 24"  fill="none"  stroke="currentColor"  stroke-width="2"  stroke-linecap="round"  stroke-linejoin="round"  class="ms-1.5"><path stroke="none" d="M0 0h24v24H0z" fill="none"/><path d="M12 5l0 14" /><path d="M16 9l-4 -4" /><path d="M8 9l4 -4" /></svg>`
                } else {
                    icon.innerHTML = `<svg  xmlns="http://www.w3.org/2000/svg"  width="13"  height="13"  viewBox="0 0 24 24"  fill="none"  stroke="currentColor"  stroke-width="2"  stroke-linecap="round"  stroke-linejoin="round"  class="ms-1.5"><path stroke="none" d="M0 0h24v24H0z" fill="none"/><path d="M12 5l0 14" /><path d="M16 15l-4 4" /><path d="M8 15l4 4" /></svg>`
                }
        ...
        if (this.deleteSelectedSelector) {
            this.deleteSelectedSelector.addEventListener("click", (e) => {
                ...
                if (selectedRows.length > 0) {
                    const { confirmButton, modalElement } = this.deleteAlert(selectedRows.length > 1 ? this.deleteMultipleRowsMessage : this.deleteRowMessage)
                    ...
                    const onConfirm = () => {
                        ...
                        this.deleteSelectedSelector.classList.add("hidden")
        ...
        if (deleteButtons) {
            deleteButtons.forEach((button) => {
                button.addEventListener("click", (e) => {
                    ...
                    const { confirmButton, modalElement } = this.deleteAlert()

                    const onConfirm = () => {
                        const rowToBeDelete = button.closest("tr")
                        if (!rowToBeDelete) return
    ...
    deleteAlert(message) {
            ...
            <div id="confirm-delete-modal" class="hs-overlay pointer-events-none fixed start-0 top-0 z-80 hidden size-full overflow-x-hidden overflow-y-auto" tabindex="-1" aria-hidden="true">
               ...
               <div class="border-default-300 flex items-center justify-between border-b p-6">
                        ...
                        <div class="overflow-y-auto p-5">   
                            <p>${message ?? this.deleteRowMessage}</p>
                        </div>
        ...
        // Remove any previous instance to avoid duplicate IDs
        ...
        // If your setup needs it for dynamic content:
        // if (window.HSOverlay?.autoInit) {
        //     window.HSOverlay.autoInit()
        // }

        // Get or create HSOverlay instance
        const modal = HSOverlay.getInstance(modalElement) || new HSOverlay(modalElement)

        HSOverlay.open(modalElement)

        const confirmButton = modalElement.querySelector("#confirm-delete")

        return { modal, confirmButton, modalElement }
    }
}
