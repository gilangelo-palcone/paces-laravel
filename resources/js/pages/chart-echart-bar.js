...
new CustomEChart({
    ...
    options: () => ({
        ...
        xAxis: {
            type: "category",
            axisLabel: {
                color: theme("default-400"), // formatter: (val) => window.dayjs(val).format("D MMM, YY"),
                fontWeight: 600,
                fontSize: 10.24,
                padding: [0, 0, 0, 20],
            },
        ...
        series: [
            {
                ...
                hoverAnimation: true,
                itemStyle: {
                    color: theme("chart-primary", 0.75),
...
new CustomEChart({
    ...
    options: () => ({
        tooltip: {
            ...
            formatter: function (params) {
                const title = params[0].name // xAxis label
                let content = `<div style="font-size: 12px; font-weight: 700; text-transform: uppercase; border-bottom: 1px solid ${theme("default-300")}; margin-bottom: 8px; padding: 3px 10px 8px;">${title}</div>`
                params.forEach((item) => {
                    content += `<div style="margin-top: 4px; padding: 3px 15px;">
                            <span style="display:inline-block;margin-right:5px;border-radius:50%;width:10px;height:10px;background-color:${item.color};"></span>
                            ${item.seriesName} : <strong>${item.value}</strong>
    ...
    options: () => ({
        baseOption: {
            timeline: {
                ...
                lineStyle: { color: theme("default-300") },
                itemStyle: { color: theme("default-300") },
                checkpointStyle: {
                    color: theme("chart-primary"),
                    shadowBlur: 0,
                    shadowOffsetX: 0,
            ...
            title: {
                textStyle: { color: theme("default-400") },
            },
            tooltip: {
                trigger: "axis",
                padding: [5, 0],
